...
##' \item{\code{A}}{A list of negative inverse community matrices}
...
##'                                          monitor=c(B=-1,C=1))))
##' @export
system.simulate0 <- function(n.sims,edges,
                             required.groups=c(0),
                             validators=NULL) {
  ...
  for(k in seq_len(n.sims)) {
    ...
    ## Check stability and validation criteria
    if(stable.community(W)) stable[k] <- T
    if(all(as.logical(lapply(validators,function(v) v(W))))) accepted[k] <- T
  ...
  list(edges=edges,W=Ws,w=ws,stable=stable,accepted=accepted)
}
