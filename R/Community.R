...
##' Subset an edge list
##'
##' These functions extract subset of an edge list containing only
##' edges in a specied group, or incident with a specified set of
##' nodes.
##' @title Edge Subsets
##' @param edges an edge list
...
##' considered optional, and may or may not be represented in the
##' community matrix.
##'
##' The \code{community} function is a function of no arguments that
##' generates a random community matrix.  The \code{select} function
##' is a function of a single argument \code{p} that determines which
##' optional edge pairs will be represented in matrices generated by
##' subsequent calls to \code{community}.  This function always
##' selects either neither or both edges of a pair and every optional
##' pair has likelihood \code{p} of being selected.  The
##' \code{weights} function is a function of a single argument
...
##' \item{\code{select(p)}}{a function that randomly selects which optional edges will be retained}
##' \item{\code{weights(W)}}{a function that returns the (non-zero) weights as a vector}
##' \item{\code{optional}}{the labels of the optional edges}
##' @examples
##' ## Sample model
##' edges <- parse.text(c(
##'   "E *-> D",
...
##' ## Do not select the D --> B edge with prob 0.8
##' s$select(0.8)
