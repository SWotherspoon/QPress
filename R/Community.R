...
##' stage determines which of the optional edges will be included or
...
##' that determines which of the optional edge pairs will be included
##' in matrices generated by subsequent calls to
##' \code{community}. This function always selects either neither or
##' both edges of a pair and every optional pair has likelihood
...
##' s$community()
##' s$community()
##' ## Select the optional D --> B edge with prob 0.6
##' s$select(0.6)
##' ## Generate community matrices
##' s$community()
##' s$community()
...
##' \item{\code{w}}{A list of vectors of the corresponding edge weights}
##' @examples
##' set.seed(32)
##' ## Sample model
##' edges <- parse.text(c(
...
system.simulate <- function(n.sims,edges,
  ...
  while(i < n.sims) {
    ...
    ## where r is the probability an optional edge is retained
    z <- s$select(runif(1))
