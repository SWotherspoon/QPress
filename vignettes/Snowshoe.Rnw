...
\section{Snowshoe Hare}
\label{sec:snowshoe-hare}
...
\begin{table}[ht]
  ...
  \begin{tabular}{lll}
    Model A &
    \begin{tikzpicture}[baseline=(current bounding box.center),->,>=stealth',auto,node distance=2cm,
...
simple textual description with the \texttt{parse.text} and
\texttt{model.text} functions.  The \texttt{parse.text} allows the
...
per line, and read and written with \texttt{model.text} and
\texttt{write.text} respectively
<<>>=
...
as a second
<<>>=
modelBCDE <- parse.text(c("V-*V","P-*P","V*->H","H*->P","V->P","H--*H","P*--H"))
modelBCDE
@
...
positive (\texttt{+}) perturbation of the $V$ node in the perturbation panel, and
pressing the update button produces the first barplot shown in
Figure~\ref{fig:Impact1}. This plot shows that a positive press
...
shows that for model B, approximately $40\%$ of the community matrices
simulated yield a decrease in hare numbers following a positive press
perturbation to the vegetation.

The third and fourth plots in Figure~\ref{fig:Impact1} show results
for the combined model BCDE.
<<eval=F>>=
simBCDE <- system.simulate(1000,modelBCDE)
impact.barplot(simBCDE)
@
Model BCDE includes uncertain edges -- edges that are included or
excluded at random in each simulated community matrix. The plot on the
lower left shows the result of selecting a positive (\texttt{+})
perturbation of the $V$ node in the perturbation panel as for the
previous models. This shows that for the combined model, it is
possible for a positive press perturbation to lead to an overall
negative impact on the vegetation. We can impose the additional
constraint that a positive press perturbation must lead to an increase
in vegetation by also selecting a positive (\texttt{+}) validation
criterion for the $V$ node in the monitor panel.  This yields the
final plot in Figure~\ref{fig:Impact1}, which shows the proportion of
positive (orange), zero (brown) and negative (blue) impacts at each
node predicted by the combined model but only for the simulated
community matrices for which a positive press perturbation to the
vegetation lead to an increase in perturbation.
\begin{figure}[ht]
  \centering
<<echo=F,fig=T>>=
...
QPress:::impact.barplot.action(node.labels(modelBCDE),simBCDE$A,c(0,0,1),c(NA,NA,1),main="Validated BCDE")
par(opar)
@
...
ease of identification, the edge selection panel is layed out in the
...
red.  For readibility, the plots are labelled by the indices that edge
...
\texttt{1:1} corresponds to the self limiting edge on \texttt{H}, and
...
simBCDE1 <- system.simulate(1000,modelBCDE,
                            validators=list(
                              ...
                              press.validate(modelBCDE,
                                             ...
                                             monitor=c(V=1))))
@
simulates community matrices consistent with model BCDE that
satisfy the validation criteria that hares decrease following a
positive press perturbation of the predators, and vegetation increases
following a press perturbation of vegetation.
