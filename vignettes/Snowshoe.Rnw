...
postive press perturbation of vegetation is observed to produce a
...
\texttt{optional.labels}.  The \texttt{select} component randomly
...
edges are given by \texttt{weight.labels} and \texttt{optional.labels}
<<>>=
s$weight.labels
s$optional.labels
@
