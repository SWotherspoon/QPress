...
\date{2013}
\author{\textbf{Simon Wotherspoon} \\
%EndAName
...
\and \textbf{Jess Melbourne-Thomas} \\
%EndAName
Australian Antarctic Division\\
Antarctic Climate \& Ecosystems CRC}
\maketitle
...
\begin{abstract}
The \textbf{QPress} package provides facilities for qualitatively modelling the 
...
analysis using qualitative network analysis. In particular, these analyses can 
...
elevated concentrations of CO$_2$ on the Southern Ocean microbial loop, 
...
directional responses of model components to increased phosphorous in the 
mesocosm experiment were increases in each of: phosphorous (Phos), large herbivores 
(Herb2), fish (Carn2) and periphyton (AlgP). \cite{JMT2012} compared the ability
...
read using \texttt{model.dia}. Here we read the two models into a list
(\texttt{lake}) using \texttt{lapply}.
<<>>=
library(QPress)
files <- c("Lake-a.dia","Lake-b.dia")
lake <- lapply(files,model.dia)
@
...
@
<<eval=F>>=
simlake.a <- system.simulate(1000,lake[[1]])
...
simlake.a <- system.simulate(1000,lake[[1]])
simlake.b <- system.simulate(1000,lake[[2]])
opar <- par(mfrow=c(1,2))
QPress:::impact.barplot.action(node.labels(lake[[1]]),simlake.a$A,c(0,0,0,0,0,0,0,1),c(NA,NA,1,NA,1,NA,1,1),main="Model (a)")
QPress:::impact.barplot.action(node.labels(lake[[2]]),simlake.b$A,c(0,0,0,0,0,0,0,1),c(NA,NA,1,NA,1,NA,1,1),main="Model (b)")
par(opar)
@
...
marine microbes from near-shore waters to elevated concentrations of CO$_2$ 
...
When this system was subjected to elevated CO$_2$ in experimental mesocosms, the
...
In the model shown in (Figure~\ref{fig:Antarctic-a}) we assumed that the direct 
effects of CO$_2$ correspond with these observed changes.
\vspace{1cm}

\begin{figure}[ht]
  \centering
  \includegraphics{Antarctic-a.pdf}
  \caption{Model representing the effects of CO$_2$ on the Southern Ocean 
...
CO$_2$. In this case we enforce a specific ordering of the vertices, and also
enforce self-limitation for each model variable.
<<>>=
## Enforce the ordering of vertices.
...
\begin{figure}[ht]
  ...
  \caption{Selection of observations for Antarctic model.}
  \label{fig:Selector2}
\end{figure}
...
\caption{Simulation outcomes in response to a positive press perturbation to the
  CO$_2$. Each plot shows the fraction of positive (orange), and negative (blue)
...
representation of the Antarctic microbial loop system. Given that increases in
dissolved CO$_2$ are generally expected to have negative impacts of the growth
of marine organisms, we could assume a direct negative effect of CO$_2$ on each
of large phytoplankton, small phytoplankton, heterortophic nanoflagellates, and 
...
\begin{figure}[ht]
  ...
  \caption{Alternative model representing the effects of CO$_2$ on the Southern  
...
@
and explorting simulation outcomes with \texttt{impact.barplot} (with the same 
...
In a third version of our microbial loop model (Figure~\ref{fig:Antarctic-c})
we therefore assume a negative effect of bacteria on DOC and a positive effect
of bacteria on macronutrients (which includes carbon).
...
@
\caption{Simulation outcomes in response to a positive press perturbation to the
  CO$_2$ for the alternative microbial loop model shown in 
...
\begin{figure}[ht]
  ...
  \caption{Model representing the effects of CO$_2$ on the Southern Ocean microbial loop.}
