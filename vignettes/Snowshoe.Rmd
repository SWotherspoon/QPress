...
knitr::opts_chunk$set(
  collapse = TRUE, warning = FALSE, message = FALSE, comment = "#>",
...
The `QPress` package provides facilities for qualitatively modelling the impact of a press perturbation upon a network model in the style of [@JMT2012].

In this document we illustrate some of the basic facilities provided by the package using the Snowshoe hare example from [@Dambacher2007].

[@Dambacher2007] describes five alternative models for the interactions between the snowshoe hare, its major predator and the supporting vegetation.  These five models are reproduced in Table 1, and show the interactions of the hare $H$ with the vegetation $V$ and predator $P$.

```{r modeldiags, echo = FALSE}
library(DiagrammeR)
str <- c("digraph m {", "rankdir=LR;", "node [shape=circle];", "V;H;P", "edge [arrowhead=dot];", "V->V", "P->P", "edge [dir=both arrowhead=normal arrowtail=dot];", "V->H", "H->P")

mA <- grViz(paste(c(str, "}"), collapse = "\n"), width = 300, height = 150)
mB <- grViz(paste(c(str, "edge [arrowhead=normal arrowtail=none];", "V->P", "}"), collapse = "\n"), width = 300, height = 150)
mC <- grViz(paste(c(str, "edge [arrowhead=normal arrowtail=none];", "V->P", "edge [arrowhead=dot];", "H->H", "}"), collapse = "\n"), width = 300, height = 150)
mD <- grViz(paste(c(str, "edge [dir=both arrowhead=normal arrowtail=dot];", "V->P", "}"), collapse = "\n"), width = 300, height = 150)
mE <- grViz(paste(c(str, "edge [dir=both arrowhead=normal arrowtail=dot];", "V->P", "edge [arrowhead=dot];", "H->H", "}"), collapse = "\n"), width = 300, height = 150)
```
...
[@Dambacher2007] note that model A predicts that a positive press perturbation of the vegetation always results in an increase in hare numbers, yet manipulation experiments have shown that this is not always the case, suggesting that model A is not an adequate description of the physical system.
...
```{r fig1, echo = FALSE, fig.cap = "**Figure 1:** Model outcomes in response to a positive press perturbation to the vegetation. Each plot shows the fraction of positive (orange), zero (brown), and negative (blue) outcomes at each node following a positive press perturbation to the vegetation."}
...
```{r fig2, echo = FALSE, fig.cap = "**Figure 2:** Edge weight density plots for a decrease in hares following a positive press perturbation to vegetation in model BCDE. Each plot corresponds to an edge, and shows the distribution of edge weights when hares decrease (blue) and when hares increase (red)."}
